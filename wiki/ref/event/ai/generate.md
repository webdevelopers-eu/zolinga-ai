...
Requests are processed in priority order — higher `priority` values (float 0–1, exclusive) are processed first. Within the same priority level, newer requests (higher `id`) are processed first. The default priority is `0.5`.

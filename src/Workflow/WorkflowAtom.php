    ...
    private function test(array $data): bool {  
        ...
        foreach ($this->validators as $validator) {
            ...
            if ($validator['pattern']) {
                ...
                $testResultInfo = 'unsupported';
                $debugInfo = "judgment generated by AI";
            }

            if ($testResult !== $validator['expect']) {
                $api->log->warning('ai', "Validation failed. Expected {$validator['expect']}, got $testResult. 👨🏻‍⚖️ $debugInfo: \"$testResultInfo\". ✏️ Text: " . json_encode($text, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
                return false;
            }
        }
        return true;

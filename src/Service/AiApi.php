...
private function processPrompt(string $ai, string $prompt, ?array $format = null, ?array $options = null): array|string
{
    ...
    if ($format === null) { // then it is serialized json
        ...
        try {
            ...
            $api->log->error('ai', "Failed to decode the model response: " . $e->getMessage() . " Response was: " . json_encode($answerRaw) . ". Request was: " . json_encode($request, JSON_PRETTY_PRINT));
